...
function trade(t){
 ...
 <p class=muted>친구와 직접 협상한 뒤 계약 내용을 입력하세요.</p>
 <select id=to>${others.map(x=>`<option value=${x.id}>${ND[x.nation][0]} · ${x.name}</option>`).join('')}</select>
 <h3>우리가 줄 것</h3>${inputs('give')}
 <h3>우리가 받을 것</h3>${inputs('want')}
 ...
 if(incoming.length)app.innerHTML+=`<section class=card><h2>📨 계약서가 왔어요</h2>
 ${incoming.map(c=>{let a=st.teams.find(x=>x.id===c.from);return `<div class=trade-item><b>${ND[a.nation][0]} · ${a.name}</b><br>우리가 받기: ${pack(c.offer)}<br>우리가 주기: ${pack(c.want)}<br><button class=yes data-id=${c.id}>✅ 계약</button><button class="no alt" data-id=${c.id}>❌ 거절</button></div>`}).join('')}</section>`;
 ...
 <details><summary><b>➕ 내 거래 올리기</b></summary><h3>내가 줄 것</h3>${inputs('pg')}<h3>받고 싶은 것</h3>${inputs('pw')}<button id=post>📢 게시하기</button></details>
 </section>`;
